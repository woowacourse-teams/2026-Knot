...
7. PR은 Issue와 영역·유형·작업자 메타데이터를 일치시키고 `#번호`로 연결한다. 자동 종료가 필요하면 `Closes #번호`를 사용한다.
8. PR 본문에는 설정 파일의 `required_pr_sections`를 포함하고, 자세한 검증·영향·ADR 맥락은 작업 복잡도에 맞게 작성한다.
