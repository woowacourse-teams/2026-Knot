...
7. 브랜치는 설정의 `branch_pattern`에 따라 `<area>/<type>/#<issue-number>`로 만들고 영역·유형 Label 및 관련 Issue 번호와 일치시킨다.
8. PR은 Issue와 영역·유형·작업자 메타데이터를 일치시키고 `#번호`로 연결한다. 자동 종료가 필요하면 `Closes #번호`를 사용한다.
9. PR 본문에는 설정 파일의 `required_pr_sections`를 포함하고, 자세한 검증·영향·ADR 맥락은 작업 복잡도에 맞게 작성한다.
10. 외부 상태를 변경하라는 요청이면 GitHub 연결 도구를 우선 사용하고, 지원되지 않을 때 인증된 `gh` CLI를 사용한다.
11. 생성 또는 수정 후 검증 스크립트를 실행하고 결과를 보고한다.
...
Draft PR은 준비 중이므로 자동 차단 대상이 아니며, Draft를 해제하기 전에 직접 검증한다.
