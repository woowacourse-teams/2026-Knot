...
> 근거: `.claude/rules/segment-pattern.md`, `.claude/rules/hook-guide.md`
...
- [ ] C-5-3. 응답 타입이 `GetFacilitiesApiResponse` 형식인가 (메서드 + 이름 + `ApiResponse`)
- [ ] C-5-4. 요청 함수가 `getFacilitiesApi` 형식인가 (목적 + 메서드 + `Api`)
- [ ] C-5-5. `async` 함수이며 **응답 데이터만 반환**하는가 (`response` 통째 반환 ❌)
...
- [ ] D-2-6. API 타입에 `Request`/`Response` 접미가 붙었는가
