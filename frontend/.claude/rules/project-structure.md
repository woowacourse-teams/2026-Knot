...
`<section />`으로 분류할 수 있을 만큼 규모가 큰, 문서의 독립적인 구획을 담당. 페이지에서 import되어 사용되며 여러 page에서 재사용 가능, 데이터 패칭부터 UI까지 하나의 End to End 플로우를 책임짐. 데이터 패칭 같은 로직을 수행하면서 하위 요소에 책임을 할당하는 지휘자 역할.
